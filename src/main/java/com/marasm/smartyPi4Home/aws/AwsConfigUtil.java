...
{
  private static final String AWS_PROPERTY_FILE_PATH = 
    System.getProperty("user.home") + "/.smartyPi4Home/aws.properties";
  ...
  {
    return getAwsProperty("client.endpoint");
  }
  public static String getClientId()
  {
    return getAwsProperty("client.id");
  }
  public static String getCertificateFilePath()
  {
    return System.getProperty("user.home") + "/.smartyPi4Home/" + getAwsProperty("certificate.file.name");
  }
  public static String getPrivateKeyFilePath()
  {
    return System.getProperty("user.home") + "/.smartyPi4Home/" + getAwsProperty("private.key.file.name");
  }
  
  private static String getAwsProperty(String name)
  {
    loadProps();
    
    String value = awsProps.getProperty(name);
    if (StringUtil.isEmpty(value))
    {
      throw new MissingResourceException("Unable to find property in " + AWS_PROPERTY_FILE_PATH, 
        AwsConfigUtil.class.getSimpleName(), name);
    }
    else
    {
      return value;
  ...
  private static void loadProps()
  {
    if (awsProps == null)
    {
      ...
      try
      {
        AppLogger.debug("Loading AWS pros from: " + AWS_PROPERTY_FILE_PATH);
        awsProps.load(new FileInputStream(AWS_PROPERTY_FILE_PATH));
      }
      catch (Exception e)
      {
